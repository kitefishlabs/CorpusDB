...
class NRTOSCParser:
    ...
    def absToOSCTimestamp(self, abs):
        return struct.pack('!LL', math.floor(abs), long(float(abs - long(abs)) * 4294967296))
    ...
    def createNRTAnalysisScore(self, sfpath='/Users/me/corpus/snd/76444.wav',
        ...
        mdpath = cwd + '/md/' + fname[0] + \
            '_' + str(tratio) + '.md.' + fname[1]

        # the two buffer alloc calls
        # oscList = [[0.0, ["/b_allocReadChannel", pBuf, sfpath, 0, -1, '[0]']]]
        oscList = [[0.0, ["/b_allocReadChannel", pBuf, sfpath, 0, -1, '[0]']]]
        oscList += [[0.01, ["/b_alloc", aBuf,
                            int(math.ceil((duration/0.04) / tratio)), self._num_analysis_features]]]
        ...
        rows = [['srcbufNum', pBuf, 'outbus', 0, 'dur', duration, 'transp', tratio], [
            'inbus', 0, 'savebufNum', aBuf, 'transp', tratio]]
        ...
        oscList += [[((duration / tratio) + 0.03),
                     ["/b_write", aBuf, mdpath, "wav", "float32"]]]
        ...
        for i, sfid in enumerate(sfsToUse):
            try:
                ...
                sfInfo[sfid] = {
                    ...
                    'params': None,
                    'tratio': node.tratio}
                oscList += [[0.0, ["/b_allocRead", runningbuffertotal,
                                   (self.corpus.anchor + '/snd/' + sfpth), 0, -1]] for sfpth in sfPaths]
                runningbuffertotal += 1
