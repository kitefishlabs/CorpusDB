...
class TestClassCreation(unittest.TestCase):
	...
	def runTest(self):
		...
		anchorpath = '/Users/kfl/dev/python/sc-0.3.1/testcorpus'
...
class TestAnalyzeSoundFile(unittest.TestCase):
	...
	def runTest(self):
		...
		self.assertAlmostEqual(corpus.rawmaps[the_node.sfid][3][0], 0.0347038, places=6)
...
class TestSoundFileUnits(unittest.TestCase):
	def runTest(self):
		...
		corpus.add_sound_file_unit(0, path=None, onset=0.0, dur=1.0) # ignore sfg for now
		...
		self.assertEqual(corpus.get_sorted_units_list(sfid)[0], corpusdb.SFU(0,1.0,0))
		self.assertEqual(corpus.get_sorted_units_list(sfid)[1], corpusdb.SFU(1.0,0.5,0))
...
class TestParentAndChildNodes(unittest.TestCase):
	...
	def runTest(self):
		...
		print sfidA
		sfidB = corpus.add_sound_file(None, None, sfidA, 1.0, sfGrpID=0, synthdef=['clipDistS'], params=[['inBus', 44, 'outBus', 46, 'mult', 5.0, 'clip', 1.0]]).sfid

		self.assertEqual(corpus.sftree.trackbacks[sfidA],['/Users/kfl/dev/python/sc-0.3.1/testcorpus/snd/36047.wav', 50.47734693877551, 1.0, 'stereoSamplerNRT'])
		
		self.assertEqual(corpus.sftree.trackbacks[sfidB][0],['clipDistS'])
		print '-------------'
		print corpus.sftree.trackbacks[sfidB][1]
		self.assertEqual(corpus.sftree.trackbacks[sfidB][1][0][4:], ['mult', 5.0, 'clip', 1.0]) # cannot test bus assignments, b/c that is determined at synthdef runtime
