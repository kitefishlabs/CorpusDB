...
class CorpusTracker:
    ...
    def __init__(self, corpus):
        ...
        self.mfccs_vars = np.reshape(
            np.var(self.mfccs[:, 1:], axis=0), (-1, 24))
...
# 		amp_lower_bound = 10.0 ** ((self.amps[threshold_unit,1] + fudge) / 20.0)
        ...
        subset_ids = np.reshape(np.argwhere(
            subset_amps[:, 1] > db_lower_bound), (-1, 1))
        # np.asarray(subset_amps[subset_ids][:,0], dtype='int32')
        return subset_ids
    ...
    def rank_units_by_euc_dist(self, target_mfccs, pool_mfccs, depth=1000, varflag=False):
        ...
        for i in range(target_mfccs.shape[0]):
            # calculate dists:
            if varflag is True:
                diffs_squared = np.square(np.subtract(
                    np.atleast_2d(target_mfccs[i, 3:16]), pool_mfccs))
                similarities = np.reshape(
                    np.sum(np.multiply(diffs_squared, self.mfccs_vars[:, 3:16]), axis=1), (-1, 1))
            else:
                print(target_mfccs[i, 3:16].shape)
                print(pool_mfccs[:, 3:16].shape)
                similarities = np.reshape(distance.euc2(np.atleast_2d(
                    target_mfccs[i, 3:16]), pool_mfccs[:, 3:16]), (-1, 1))
            ...
            id_ranking = np.append(np.atleast_2d(
                id_ranking), pool_mfccs[similarities][:, 0])
    ...
    def rerank_units_by_concat_cost(self, t_minus_one_mfccs, pool_mfccs, limit=-1):
        costs = np.reshape(distance.euc2(np.atleast_2d(
            t_minus_one_mfccs[:, 3:16]), pool_mfccs[:, 3:16]), (-1, 1))
    ...
    def rerank_units_by_amp(self, target_id, pool_ids, limit=None):
        ...
        target_amps = self.amps_for_ids(
            np.array([target_id], dtype=np.int32))[:, 1]

        diffs = np.reshape(distance.euc2(np.atleast_2d(target_amps), np.atleast_2d(
            self.amps_for_ids(pool_ids)[:, 1]).T), (-1, 1))
    ...
    def rerank_units_by_amp_continuity(self, target_id_t_minus_1, target_id_t_plus_1, pool_ids, limit=None):
        ...
        target_amp_t_minus_1 = self.amps_for_ids(
            np.array([target_id_t_minus_1], dtype=np.int32))[:, 1][0]
        target_amp_t_plus_1 = self.amps_for_ids(
            np.array([target_id_t_plus_1], dtype=np.int32))[:, 1][0]
    ...
    def filter_units_by_amp_threshold_old(self, target_ids, ranked_ids, db_threshold=-3, limit=25):
        ...
        for i in range(target_ids.shape[0]):
            print("> ", target_ids[i])
            ranked_with_amps_gt += np.reshape(ranked_ids[i][self.amps_greater_than(
                ranked_ids[i], target_ids[i], fudge=db_threshold)], (1, -1))[:, :limit].tolist()
    ...
    def select_rg(self, type='urg', size=100):
        if type is 'urg':
            ...
            weights = [((1.0-(float(x/float(size))))**2.0)
                       for x in range(size)]
